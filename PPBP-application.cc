...
namespace ns3 {
	...
	PPBPApplication::GetTypeId (void)
	{
		...
		.AddAttribute ("MeanBurstArrivals", "Mean Active Sources",
					   RandomVariableValue (ConstantVariable (20)),
 					   MakeRandomVariableAccessor (&PPBPApplication::m_burstArrivals),
					   MakeRandomVariableChecker ())
		.AddAttribute ("MeanBurstTimeLength", "Pareto distributed burst durations",
					   RandomVariableValue (ConstantVariable (0.2)),
					   MakeRandomVariableAccessor (&PPBPApplication::m_burstLength),
					   MakeRandomVariableChecker ())
	...
	PPBPApplication::PPBP() // Poisson Pareto Burst 
	{
		...
		ExponentialVariable exp(inter_burst_intervals);
